    ...
    axios.get(url).then((response) => {
        if(response.status === 200){
            ...
            res.send(`
                ...
                <h2>Enlaces</h2>
                <ul>
                    ${links.map(link => `<li><a href="https://carlosdiazgirol.github.io/dashboard/${link}">${link}</a></li>`).join('')}
                </ul>
                <h2>Imagenes</h2>
                <ul>
                    ${imgs.map(img =>  `<li><a href="https://carlosdiazgirol.github.io/dashboard/${img}">${img}</a></li>`).join('')}
                </ul>
            `);             
        }
    })
